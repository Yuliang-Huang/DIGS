...
sys.path.append("./")
from digs.utils.ct_utils import get_geometry_tigre, get_geometry_rtk, getFOVMask, recon_volume
from digs.arguments import ParamGroup, ModelParams, PipelineParams
from digs.gaussian import GaussianModel, query, initialize_gaussian
...
class InitParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.density_thresh = 0.001 #0.005
...
def main(
    ...
    if not save_path:
        save_path = osp.join(
            osp.dirname(data_path), "init_" + osp.basename(data_path).split(".")[0] + ".npy"
        )
    ...
    cgls_file_path = osp.join(
            osp.dirname(save_path), f"{osp.basename(data_path).split('.')[0]}_cgls.npy"
        )
