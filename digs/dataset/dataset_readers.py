...
sys.path.append("./")
from digs.utils.graphics_utils import BasicPointCloud, fetchPly
...
def readNAFInfo(path, eval):
    ...
    with open(path, "rb") as f:
        data = pickle.load(f)
    # ! unit: mm; detector: [u,v]; voxel: [x,y,z]
    scanner_cfg = {
        "DSD": data["DSD"],
        "DSO": data["DSO"],
        "nVoxel": data["nVoxel"],
        ...
        "offDetector": (np.array(data["offDetector"])).tolist(), #! varied offset for different views
        "mode": data["mode"]
    }
    ...
    for split in splits:
        ...
        for i_split in range(n_split):
            ...
            # Note, dDetector is [u, v]
            FovX = np.arctan2(scanner_cfg["sDetector"][0] / 2, scanner_cfg["DSD"]) * 2
            FovY = np.arctan2(scanner_cfg["sDetector"][1] / 2, scanner_cfg["DSD"]) * 2
            ...
            cam_info = CameraInfo(
                ...
                width=scanner_cfg["nDetector"][0],
                height=scanner_cfg["nDetector"][1],
                mode=mode,
